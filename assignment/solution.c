 ...
 *
 * These are parallel arrays, so use them both with the same index to make upy
...
// since x will be the power of 2 and x != 1, so x >= 2
int my_log2(int x) {
    // if the input x is less than 0, return -1 (wrong code)
    int result = -1;
    while (x > 0){
        x>>=1;
        result++;
...
int main(int argc, char* argv[]) {
	...
	MPI_Bcast(menu_prices, ITEMS_ON_MENU, MPI_INT,
	...
	//long total_cost_per_menu_item[ITEMS_ON_MENU] = {0};
	// TODO: change this call to use point-to-point communication instead!
//	MPI_Reduce(
//		cost_per_menu_item, total_cost_per_menu_item,
//		ITEMS_ON_MENU, MPI_LONG, MPI_SUM,
//		ROOT_PROCESS, MPI_COMM_WORLD
//	);
    ...
    for (long i = 1; i<=power; i++){
        long involvedProcNumber = size/(my_pow(2,i));
...
//            MPI_Recv(recvData, ITEMS_ON_MENU, MPI_LONG, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
//            for (int ii=0; ii<ITEMS_ON_MENU; ii++){
//                cost_per_menu_item[ii] += recvData[ii];
//            }
//        }
//    }else{
//        MPI_Send(cost_per_menu_item, ITEMS_ON_MENU, MPI_LONG, ROOT_PROCESS, 0, MPI_COMM_WORLD);
//    }
...
static int parse_order(int ** restrict item, int ** restrict quantity) {
	...
	// Note: I tried using stat + mmap and it was identical in performance to
	// stdio. So don't bother optimizing this:
